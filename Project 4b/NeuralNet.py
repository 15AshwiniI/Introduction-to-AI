...
class Perceptron(object):
    ...
    def sigmoidDeriv(self, value):
        ...
        """
        """YOUR CODE"""
        return (1-self.sigmoid(value))*self.sigmoid(value)
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        updatedWeights = []
        inActs = [1.0] + inActs
        for index, val in enumerate(inActs):
            updatedWeight = (delta*alpha*val)+self.weights[index]
            totalModification += abs(updatedWeight - self.weights[index])
...
class NeuralNet(object):                                    
    ...
    def feedForward(self, inActs):
        ...
        """YOUR CODE"""
        network = []
        network.append(inActs)
        currLayer = list(inActs)
        for layer in self.layers:
            nextLayer = []
            for node in layer:
                nextLayer.append(node.sigmoidActivation(currLayer))
    ...
    def backPropLearning(self, examples, alpha):
        ...
        for example in examples:#for each example
            ...
            for outputNum in xrange(len(example[1])):
                ...
                error = example[1][outputNum]-allLayerOutput[-1][outputNum]
                delta = error*gPrime
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                lengthLayer = len(layer)
                for neuronNum in xrange(lengthLayer):
                    gPrime = layer[neuronNum].sigmoidActivationDeriv(allLayerOutput[layerNum])
                    nextLayerLength = len(nextLayer)
                    delta = sum([nextLayer[index].weights[neuronNum+1]*deltas[0][index] for index in xrange(nextLayerLength)])*gPrime
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    while iteration < maxItr and weightMod > weightChangeThreshold:
        result = nnet.backPropLearning(examplesTrain, alpha)
        trainError = result[0]
        weightMod = result[1]
        iteration += 1
    """
    ...
    testError = 0
    testCorrect = 0 
    for x, knownList in examplesTest:
        output = nnet.feedForward(x)[-1] # last layer
        lengthOfOutput = len(output)
        correctExample = True
        for val in xrange(lengthOfOutput):
            if knownList[val] != round(output[val]):
                correctExample = False
                testError += 1
                break
        if correctExample:
            testCorrect += 1
    ...
    testAccuracy = testCorrect / (testCorrect + float(testError))
