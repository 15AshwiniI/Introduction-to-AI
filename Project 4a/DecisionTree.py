...
def getClassCounts(examples,className):
    ...
    for e in examples:
        if e[className] not in classCounts.keys():
            # Initialize the classCounts value
...
def getAttributeCounts(examples,attrName,attrValues,className):
    ...
    for val in attrValues:
        attributeCounts[val] = getClassCounts(getPertinentExamples(examples, attrName, val), className)
...
def setEntropy(classCounts):
    ...
    #YOUR CODE HERE
    totalEntropy = 0.0
    # use python's sum function for list
    numCounts = sum(classCounts)
    for count in classCounts:
        # Log is imported at the top
        c = float(count)/numCounts
        totalEntropy -= c*log(c,2)
    return totalEntropy
...
def remainder(examples,attrName,attrValues,className):
    ...
    #YOUR CODE HERE
    return_value = 0
    for c in getAttributeCounts(examples, attrName, attrValues, className).values():
        return_value += (float(sum(c.values()))/len(examples))*setEntropy(c.values())
    return return_value
...
def giniIndex(classCounts):
    ...
    for c in classCounts:
        return_value += -((float(c)/sum(classCounts))*(float(c)/sum(classCounts)))
    return return_value
...
def giniGain(examples,attrName,attrValues,className):
    ...
    summedGiniScore = 0
    import sys
    exampleLength = len(examples)
    for c in getAttributeCounts(examples, attrName, attrValues, className).values():
        summedGiniScore += giniIndex(c.values())*(float(sum(c.values()))/exampleLength)
    return sys.maxint if summedGiniScore == 0 else 1/summedGiniScore
...
def makeSubtrees(remainingAttributes,examples,attributeValues,className,defaultLabel,setScoreFunc,gainFunc):
    ...
    """
    #YOUR CODE HERE
    if len(remainingAttributes) == 0 or len(examples) == 0:
        return LeafNode(defaultLabel)
    elif len(getClassCounts(examples, className)) == 1:
        return LeafNode(examples[0][className])

    b = remainingAttributes[0]
    for a in remainingAttributes[1:]:
        if gainFunc(examples, a, attributeValues[a], className)>gainFunc(examples, b, attributeValues[b], className):
            b = a

    editedRemainingAttributes = list(remainingAttributes)
    editedRemainingAttributes.remove(b)

    rootNode = Node(b) # Not a leaf node for this one
    for val in attributeValues[b]:
        pExamples = getPertinentExamples(examples, b, val)
        rootNode.children[val] = makeSubtrees(editedRemainingAttributes, pExamples, attributeValues, className, getMostCommonClass(examples,className), setScoreFunc, gainFunc)
    return rootNode
