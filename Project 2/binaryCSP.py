...
"""
	...
	Satisfied if values are not 1 unit away from each other
"""
class NotNextDiagonal(BinaryConstraint):
	def isSatisfied(self, value1, value2):
		if abs(int(value2)-int(value1)) == 1: 
			return False
		else:
			return True

	def __repr__(self):
		return 'NotNextDiagonal (%s, %s)' % (str(self.var1), str(self.var2))
	
"""
	Implementation of BinaryConstraint
	Satisfied if values are not 2 unit away from each other
"""
class NotTwoDiagonal(BinaryConstraint):
	def isSatisfied(self, value1, value2):
		if abs(int(value2)-int(value1)) == 2: 
			return False
		else:
			return True

	def __repr__(self):
		return 'NotTwoDiagonal (%s, %s)' % (str(self.var1), str(self.var2))

"""
	Implementation of BinaryConstraint
	Satisfied if values are not 3 away from each other
"""
class NotThreeDiagonal(BinaryConstraint):
	def isSatisfied(self, value1, value2):
		if abs(int(value2)-int(value1)) == 3: 
			return False
		else:
			return True

	def __repr__(self):
		return 'NotThreeDiagonal (%s, %s)' % (str(self.var1), str(self.var2))
"""
	Implementation of BinaryConstraint
	Satisfied if values are not 4 away from each other
"""
class NotFourDiagonal(BinaryConstraint):
	def isSatisfied(self, value1, value2):
		if abs(int(value2)-int(value1)) == 4: 
			return False
		else:
			return True

	def __repr__(self):
		return 'NotFourDiagonal (%s, %s)' % (str(self.var1), str(self.var2))
...
def consistent(assignment, csp, var, value):
	...
	for bc in csp.binaryConstraints:
		if bc.affects(var):
			otherV = bc.otherVariable(var)
			if assignment.isAssigned(otherV):
				if not (bc.isSatisfied(value, assignment.assignedValues[otherV])):
					return False
	return True
...
def recursiveBacktracking(assignment, csp, orderValuesMethod, selectVariableMethod):
	...
	if assignment.isComplete():
		return assignment
	myVar = selectVariableMethod(assignment, csp)
	for currVal in orderValuesMethod(assignment, csp, myVar):
		if consistent(assignment, csp, myVar, currVal):
			assignment.assignedValues[myVar] = currVal
			result = recursiveBacktracking(assignment, csp, orderValuesMethod, selectVariableMethod)
			if not result == None:
				return result
			assignment.assignedValues[myVar] = None
	return None
...
def minimumRemainingValuesHeuristic(assignment, csp):
	...
	"""Question 2"""
	"""YOUR CODE HERE"""
	""" select variable with fewest options left"""
	for variable, domain in domains.iteritems():
		if not assignment.isAssigned(variable):
			numberOfBinaryConstraintsCurrVar = 0
			numberOfBinaryConstraintsNextVar = 0
			for bConst in csp.binaryConstraints:
				if bConst.affects(variable):
					numberOfBinaryConstraintsCurrVar += 1
				if bConst.affects(nextVar):
					numberOfBinaryConstraintsNextVar += 1
			if (nextVar == None):
				nextVar = variable
			else:
				numDomain = len(domain)
				numDomainNextVar = len(domains[nextVar])
				if (numDomain < numDomainNextVar):
					nextVar = variable
				if (numDomain == numDomainNextVar):
					if (not variable == nextVar):
						if numberOfBinaryConstraintsCurrVar > numberOfBinaryConstraintsNextVar:
							nextVar = variable
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
	...
	"""Question 3"""
	# So the list needs to be sorted by values with the least constraining value first.
	retVal = []
	for item in values:
		counter = 0
		for bConst in csp.binaryConstraints:
			if bConst.affects(var):
				oVar = bConst.otherVariable(var)
				domains = assignment.varDomains[oVar]
				for dom in domains:
					if bConst.isSatisfied(item, dom):
						pass
					else:
						counter += 1
		retVal.append((counter, item))
	retVal.sort()
	returnValue = []
	for val in retVal:
		returnValue.append(val[1])
	return returnValue
...
def forwardChecking(assignment, csp, var, value):
	...
	for bConst in csp.binaryConstraints:
		if bConst.affects(var):
			oVar = bConst.otherVariable(var)
			for dom in domains[oVar].copy():
				if bConst.isSatisfied(value, dom):
					pass
				else:
					inferences.add((oVar, dom))
					domValues = domains[oVar]
					domValues.remove(dom)
					numDomainsOtherVars = len(domValues)
					if numDomainsOtherVars < 1:
						for pair in inferences:
							myVar = pair[0]
							myVal = pair[1]
							domains[myVar].add(myVal)
						return None
	return inferences
...
def recursiveBacktrackingWithInferences(assignment, csp, orderValuesMethod, selectVariableMethod, inferenceMethod):
	...
	for currVal in orderValuesMethod(assignment, csp, myVar):
		if consistent(assignment, csp, myVar, currVal):
			assignment.assignedValues[myVar] = currVal
			infChoice = inferenceMethod(assignment, csp, myVar, currVal)
			if infChoice is not None:
				for pair in infChoice:
					var = pair[0]
					val = pair[1]
...
def revise(assignment, csp, var1, var2, constraint):
	...
	domains = assignment.varDomains
	retNone = False
	for firstVariable in domains[var1].copy():
		valid = False
		for secVariable in domains[var2]:
			valid += constraint.isSatisfied(firstVariable, secVariable)
		if not valid:
			domains[var1].remove(firstVariable)
			if len(domains[var1]) < 1:
				for pair in inferences:
					x, y = pair
					domains[x].add(y)
				retNone = True
			inferences.add((var1, firstVariable))
		else:
			pass
		if retNone:
			return None
	for secVariable in domains[var2].copy():
		valid = False
		for firstVariable in domains[var1]:
			valid += constraint.isSatisfied(firstVariable, secVariable)
		if not valid:
			domains[var2].remove(secVariable)
			if len(domains[var2]) < 1:
				for pair in inferences:
					x, y = pair
					domains[x].add(y)
				retNone = True
			inferences.add((var2,secVariable))
	if retNone:
		return None
	else:
		return inferences
...
def maintainArcConsistency(assignment, csp, var, value):
	...
	for bConst in csp.binaryConstraints:
		if bConst.affects(var):
			validConstraints.append(bConst)
	queue = deque(validConstraints)
	return arcHelper(assignment, csp, queue, 0)

"""
	My helper function for AC3 and arc consistency logic
...
"""
def arcHelper(assignment, csp, queue, retType):
	inferences = set([])
	while len(queue) > 0:
		poppedNode = queue.pop()
		moreInfer = revise(assignment, csp, poppedNode.var1, poppedNode.var2, poppedNode)
		if moreInfer is None:
			for pair in inferences:
				x, y = pair
				assignment.varDomains[x].add(y)
			return None
		if len(moreInfer) != 0:
			inferences = inferences.union(moreInfer)
			for bConst in csp.binaryConstraints:
				if bConst.affects(poppedNode.var1) or bConst.affects(poppedNode.var2):
					queue.append(bConst)
	if retType == 0:
		return inferences
	else:
