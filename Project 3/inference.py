...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        timeStep = util.Counter()
        for pos in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState,pos))
            for newPos, prob in newPosDist.items():
                timeStep[newPos] = self.beliefs[pos]*prob + timeStep[newPos]
        timeStep.normalize()
        self.beliefs = timeStep
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        """
          ...
          ** NOTE **
            the variable you store your particles in must be a listÍ¾ a list is simply a collection
        ...
        "*** YOUR CODE HERE ***"
        particles = []
        numberOfPos = len(self.legalPositions)
        for k in range(self.numParticles):
            # if k < numberOfPos:
            #     particles.append(self.legalPositions[k])
            # else:
            particles.append(self.legalPositions[k % numberOfPos])
        ...
        if noisyDistance is not None:
            ...
            for particle in self.particles:
                ...
                if emissionModel[distance] > 0:
                    currentBelief[particle] = emissionModel[distance] + currentBelief[particle]
            # currentBelief.normalize() # Normalizing in getBeliefDistribution
            if currentBelief.totalCount() == 0:
                self.initializeUniformly(gameState)
            else:
                newParticles = []
                for _ in range(len(self.particles)):
                    newParticles.append(util.sample(currentBelief))
    ...
    def elapseTime(self, gameState):
        ...
        updated_particles = []
        for particle in self.particles:
            newPartPos = self.getPositionDistribution(self.setGhostPosition(gameState, particle))
            pos = util.sample(newPartPos)
            updated_particles.append(pos)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        itertools, keep in mind that permutations are not returned in a random orderÍ¾
        ...
        "*** YOUR CODE HERE ***"
        particles = []
        # This is how itertools product works
        # list(itertools.product([1,2,3],repeat = 2))
        # I need to do it for the legal positions and repeat for the number of ghosts
        permutations = list(itertools.product(self.legalPositions, repeat = self.numGhosts))
        random.shuffle(permutations)
        numberOfPos = len(permutations)
        for k in range(self.numParticles):
            # if k < numberOfPos:
            #     particles.append(self.legalPositions[k])
            # else:
            # particles.append(permutations[k % numberOfPos])
            # But they store particles as tuples?
            particles.append(tuple(permutations[k % numberOfPos]))
        self.particles = particles
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        for iGhost in range(self.numGhosts):
            if noisyDistances[iGhost] is None:
                newParticles = []
                for particle in self.particles:
                    newParticles.append(self.getParticleWithGhostInJail(particle, iGhost))
                self.particles = newParticles

        for particle in self.particles:
            currentUpdate = 1
            for iGhost in range(self.numGhosts):
                distance = util.manhattanDistance(particle[iGhost], pacmanPosition)
                if noisyDistances[iGhost] is None:
                    # print 'last test case'
                    pass
                else:
                    currentUpdate = emissionModels[iGhost][distance] * currentUpdate
            currentBelief[particle] += currentUpdate
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            updateParticle = []
            for iGhost in range(self.numGhosts):
                newPartPos = getPositionDistributionForGhost(setGhostPositions(gameState, oldParticle), iGhost, self.ghostAgents[iGhost])
                updateParticle.append(util.sample(newPartPos))
            newParticle = updateParticle
