...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        #self.distances.geâ€ Distance(pos1,pos2)
        ...
        # Iterate through list of dictionaries picking most likely positions for each ghost
        likelyPos = []
        for ghost in livingGhostPositionDistributions:
            maxProb = -1 # Everything should be normalized and under <=1 and >=0
            maxKey = None
            for key, val in ghost.iteritems():
                if val > maxProb:
                    maxProb = val
                    maxKey = key
            likelyPos.append(maxKey)

        # Find the closest ghost
        closestDist = float("inf")
        closestGhost = None
        for ghostPos in likelyPos:
            currDist = self.distancer.getDistance(ghostPos, pacmanPosition)
            if (currDist < closestDist):
                closestDist = currDist
                closestGhost = ghostPos

        # Find the action to move to that closest ghost
        closest = float("inf")
        bestAction = None
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            currDist = self.distancer.getDistance(closestGhost, successorPosition)
            if (currDist < closestDist):
                closestDist = currDist
                bestAction = action
        return bestAction